...
class AlarmService : Service(), TextToSpeech.OnInitListener {
    ...
    fun stopAlarm() { if (!active) { stopSelf(); return }; active = false; handler.removeCallbacksAndMessages(null); mediaPlayer?.release(); mediaPlayer = null; tts?.stop(); setTorch(false); writeLog("ALARM DEACTIVATED"); stopForeground(STOP_FOREGROUND_REMOVE); stopSelf() }
